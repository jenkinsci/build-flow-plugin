package edu.washington.cac.nms.alertpub;
...
/**
 * <p>DirectedAcyclicGraph implements a DAG that can be modified (vertices &amp; edges added 
 * and removed), is guaranteed to remain acyclic, and provides fast topological order iteration. 
 * </p>   
 *
 * <p>This is done using a dynamic topological sort which is based on 
 * the algorithm PK described in "D. Pearce &amp; P. Kelly, 2007: A Dynamic  Topological Sort 
 * Algorithm for Directed Acyclic Graphs", (see 
 ...
 * </p> 
 *
 * <p>The implementation differs from the algorithm specified in the above paper in some 
 * ways, perhaps most notably in that the topological ordering is stored by default using 
 * two HashMaps, which will have some effects on runtime, but also allows for 
 * vertex addition and removal, and other operations which are helpful for manipulating 
 * or combining DAGs.  This storage mechanism is pluggable for subclassers. 
 * </p> 
 *
 * <p>This class makes no claims to thread safety, and concurrent usage from multiple 
 * threads will produce undefined results. 
 * </p> 
 *
 * @author Peter Giles, [hidden email] 
 *
 * @param <V> the vertex class, which must be suitable for use as a hash key 
...
public class DirectedAcyclicGraph<V, E> extends SimpleDirectedGraph<V, E> {
    ...
    private static Log log = LogFactory.getLog(DirectedAcyclicGraph.class);
     ...
     * vertex vb then va is garanteed to come before vertex vb in the iteration
     * order.
     ...
     * @throws IllegalArgumentException If either fromVertex or toVertex is not a member of the graph
    ...
    public E addDagEdge(V fromVertex, V toVertex) throws CycleFoundException {
        ...
        Integer lb = topoOrderMap.getTopologicalIndex(toVertex); //vertexToTopo.get(toVertex);
        Integer ub = topoOrderMap.getTopologicalIndex(fromVertex); //vertexToTopo.get(fromVertex);
        ...
        if (lb < ub) {
            ...
            ++topologyUpdateCount; // if we do a reorder, than the topology has been updated
    ...
    /**
     * identical to {@link #addDagEdge(Object, Object)}, except an unchecked {@link IllegalArgumentException}
     * is thrown if a cycle would have been induced by this edge
     */
    @Override
    ...
    /**
     * identical to {@link #addDagEdge(Object, Object, Object)}, except an unchecked {@link IllegalArgumentException}
     * is thrown if a cycle would have been induced by this edge
     */
    @Override
    ...
    // note that this can leave holes in the topological ordering, which (depending
    // on the TopoOrderMap implementation) can degrade performance for certain operations
    // over time
    @Override
     ...
     * @param visited a simple data structure that lets us know if we already visited a node with a given topo index
     * @param topoIndexMap for quick lookups, a map from vertex to topo index in the AR
     * @param ub the topo index of the original fromVertex -- used for cycle detection
    ...
    private void dfsF(V vertex, Set<V> df, Visited visited, Region affectedRegion) throws CycleFoundException {
        int topoIndex = topoOrderMap.getTopologicalIndex(vertex); // vertexToTopo.get(vertex);
        ...
        for (E outEdge : outgoingEdgesOf(vertex)) {
            ...
            Integer nextVertexTopoIndex = topoOrderMap.getTopologicalIndex(nextVertex); //vertexToTopo.get(nextVertex);
            ...
            if (nextVertexTopoIndex.intValue() == affectedRegion.finish) {
                ...
                try {
                    ...
                    // okay, fine, some implementations (ones that automatically clear themselves out)
                    // don't work this way
                }
                throw new CycleFoundException();
            }

            // note, order of checks is important as we need to make sure the vertex is in the affected region before we
            // check its visited status (otherwise we will be causing an ArrayIndexOutOfBoundsException).
    ...
    private void dfsB(V vertex, Set<V> db, Visited visited, Region affectedRegion) {
        ...
        for (E inEdge : incomingEdgesOf(vertex)) {
            ...
            Integer previousVertexTopoIndex = topoOrderMap.getTopologicalIndex(previousVertex); // vertexToTopo.get(previousVertex);

            // note, order of checks is important as we need to make sure the vertex is in the affected region before we
            // check its visited status (otherwise we will be causing an ArrayIndexOutOfBoundsException).
            if (affectedRegion.isIn(previousVertexTopoIndex) && !visited.getVisited(previousVertexTopoIndex)) {
                // if prevousVertexTopoIndex != null, the vertex is in the Affected Region
                // according to our topoIndexMap
    ...
    private void reorder(Set<V> df, Set<V> db, Visited visited) {
        ...
        // we have to cast to the generic type, can't do "new V[size]" in java 5;
        V[] bigL = (V[]) new Object[df.size() + db.size()];
        int lIndex = 0; // this index is used for the sole purpose of pushing into
        ...
        for (V vertex : topoDb) {
            Integer topoIndex = topoOrderMap.getTopologicalIndex(vertex); // vertexToTopo.get(vertex);
    ...
    /**
     * Note, this is a lazy and incomplete implementation, with assumptions
     * that inputs are in the given topoIndexMap
     * @author gilesp
     *
     * @param <V>
     */
    ...
    /**
     * For performance tuning, an interface for storing the
    ...
    public interface TopoOrderMapping<V> extends Serializable {
         ...
         * @param vertex
         * @return the index that the vertex is at, or null if the vertex isn't in the
         * topological ordering
         */
         ...
         *
         * @param vertex
         * @return the index that the vertex was at, or null if the vertex wasn't in the
         * topological ordering
         */
    ...
    public class TopoVertexMap  implements TopoOrderMapping<V>, TopoOrderMappingFactory<V> {
        ...
        /**
         * We translate the topological index to an ArrayList index.  We have
         * to do this because topological indices can be negative, and we want
         * to do it because we can make better use of space by only needing
         * an ArrayList of size |AR|.
         * @param unscaledIndex
    ...
    public static class Region implements Serializable {
        ...
        public Region(int start, int finish) {
            if (start > finish) throw new IllegalArgumentException("(start > finish): invariant broken");
            this.start = start;
            this.finish = finish;
        }
    ...
    /**
     * This implementation seems to offer the best performance in most cases.  It grows
     * the internal ArrayList as needed to be as large as |AR|, so it will be more memory
     * intensive than the HashSet implementation, and unlike the Array implementation, it
     * will hold on to that memory (it expands, but never contracts).
     * @author gilesp
     *
     */
...
//	 } catch (IndexOutOfBoundsException e) { 
//	 log.warn("clearing visited out of the region boundaries", e); 
...
//	 } 
    ...
    /**
     * This implementation doesn't seem to perform as well, though I can imagine circumstances
     * where it should shine (lots and lots of vertices).  It also should have the lowest memory
     * footprint as it only uses storage for indices that have been visited.
     * @author gilesp
     */
    ...
    /**
     * This implementation, somewhat to my surprise, is slower than the ArrayList version,
     * probably due to its reallocation of the underlying array for every topology reorder
     * that is required.
     * @author gilesp
     *
     */
    ...
    /**
     * this interface  allows specification of a strategy for marking vertices as visited
     * (based on their topological index, so the vertex type isn't part of the interface).
     */
    public interface Visited {
         ...
         * @param index
         * @throws UnsupportedOperationException if the implementation doesn't support (or doesn't need) clearance.
         * For example, if the factory vends a new instance every time, it is a waste of cycles to clear the state
         * after the search of the Affected Region is done, so an UnsupportedOperationException *should* be thrown.
         */
